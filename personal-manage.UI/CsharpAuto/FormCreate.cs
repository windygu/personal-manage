...
namespace personal_manage.UI.CsharpAuto
{
    public partial class FormCreate : Form
    {
        ...
        /// 表名字典
        /// </summary>
        public static Dictionary<string,TableInfo> tablesDicts ;
        ...
        {
            string sqlAllTable = "SELECT name FROM sqlite_master WHERE type='table' and name!='sqlite_sequence' ";

            if (!string.IsNullOrEmpty(keyWord.Text))
            {
                sqlAllTable += $" and name like '%{keyWord.Text}%'";
            }
            sqlAllTable += " ORDER BY name";

            DataTable  dataTable = createBLL.SelectData(sqlAllTable);
            CheckBox checkBox;
            ...
            for (int i = 0; i < dataTable.Rows.Count; i++)
            {
                ...
                checkBox.Tag = curRow;
                checkBox.Size = new Size(290, 25);
                checkBox.Text = dataTable.Rows[i].Field<string>("name");
                this.panel2.Controls.Add(checkBox);
            }
        }
        ...
        private void SaveBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < tableArr.Length; i++)
                {
                    ...
                    {
                        projectCodePath = templateConfigInfos[k].TemplatePath.Replace("{tableName}", "");
                        outFileFolder = templateConfigInfos[k].TemplatePath.Replace("{tableName}", csProjectCode.EntityName);
